...
class GeminiClient:
    def __init__(self, model: str, api_key: str):
        ...
        os.environ["GOOGLE_API_KEY"] = api_key  # helps list_models consistency
        genai.configure(api_key=api_key)
        self.model_name = model

    def generate(self, system_prompt: str, emotion_hint: str,
                 history: ChatHistory, max_tokens: int = 160):
        """
        Robust generate:
        - Use a single-turn prompt (last user only) to avoid chat parsing quirks.
        - Lower safety thresholds so benign content isn't blocked.
        - Try both bare and 'models/...' names.
        - Return (text, finish_reason, blocked_info)
        """
        def pick_last_user(h: ChatHistory) -> str:
        ...
        safety_settings = [
            ...
            dict(category=HarmCategory.HARM_CATEGORY_HATE_SPEECH,
                 threshold=HarmBlockThreshold.BLOCK_MEDIUM_AND_ABOVE),
            dict(category=HarmCategory.HARM_CATEGORY_HARASSMENT,
                 threshold=HarmBlockThreshold.BLOCK_MEDIUM_AND_ABOVE),
            dict(category=HarmCategory.HARM_CATEGORY_SEXUALLY_EXPLICIT,
                 threshold=HarmBlockThreshold.BLOCK_MEDIUM_AND_ABOVE),
        ]
        ...
        names_to_try = [self.model_name]
        if not self.model_name.startswith("models/"):
            names_to_try.append(f"models/{self.model_name}")

        last_finish, blocked_info = None, None
        for name in names_to_try:
            try:
                ...
                if not text and getattr(resp, "candidates", None):
                    ...
                    # capture safety context if any
                    sr = getattr(cand, "safety_ratings", None)
                    if sr:
                        blocked_info = [str(r) for r in sr]

                print(f"[LLM] via {name}: has_text={bool(text)} finish={last_finish!r}")
                if text:
                    return text, last_finish, None
            except Exception as e:
                print(f"[LLM] EXC via {name}:", repr(e))

        # nothing usable
        return "", last_finish, blocked_info
...
class EmotionApp(QWidget):
    ...
    def record_and_predict(self):
        ...
        try:
            ...
            bot_text, finish,  blocked = "", None, None
            if self.llm is not None:
                bot_text, finish, blocked = self.llm.generate(system_prompt, emotion_hint, self.history, max_tokens=160)
            ...
            if not bot_text:
                ...
                dbg = f"(Gemini empty; finish={finish!r}{' blocked' if blocked else ''})"
                self._add_msg(dbg, is_user=False)

                # one-line *minimal* fallback without quoting the whole transcript
