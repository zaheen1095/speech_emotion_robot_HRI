...
def _build_val_from_train(val_fraction=0.20, seed=42):
    ...
    _, val_idx = next(gss.split(paths, labels, groups))
    Xv_all = [paths[i] for i in val_idx]
    yv_all = [labels[i] for i in val_idx]
...
def _ece(probs, y_true, n_bins=15):
    conf = probs.max(1); preds = probs.argmax(1); acc = (preds == y_true).astype(float)
...
def _reliability_diagram(probs, y_true, out_png, n_bins=15):
    ...
    for i in range(n_bins):
        lo, hi = bins[i], bins[i+1]; sel = (conf >= lo) & (conf < hi)
        if sel.any():
            mids.append((lo+hi)/2); accs.append(acc[sel].mean()); confs.append(conf[sel].mean())
...
def main():
    ...
    def _closure():
        ...
        loss = torch.nn.functional.cross_entropy(Lv / T.clamp(min=1e-6), yv_t)
        loss.backward()
        return loss
    ...
    with open(out/"temperature.json","w") as f:
        json.dump({
            "temperature": T_star,     # <-- key matches test_model.py reader
            "val_samples": len(yv),
            "metrics_test": {
